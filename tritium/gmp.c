...
#define B32 1
#else
#error LONG_BIT is defined and is not 32 or 64
#endif
#endif
...
#ifdef DEBUG
        printf("k=%d\n",k);
        printf("t=%s\n",mpz_get_str(NULLPTR,10,&t));
        printf("x=%s\n",mpz_get_str(NULLPTR,10,x));
        printf("m=%s\n",mpz_get_str(NULLPTR,10,&m));
#endif
    }
    if (x->sn)
        x->sn = sn;
...
MP_INT *x; unsigned int size;
{
    ...
    unsigned int ndigits = bits/DIGITBITS;
    unsigned int oflow = bits % DIGITBITS;
    unsigned int i,j;
    long t;
    if (oflow)
        ndigits++;
    _mpz_realloc(x,(mp_size)ndigits);
    ...
    if (oflow)
        (x->p)[ndigits-1] &= (((mp_limb)1 << oflow) - 1);
}
...
void mpz_sqrt (root, a)
    ...
    MP_INT *a;
{
    mpz_sqrtrem (root, NULLPTR, a);
}

/* RDB: -- extras */
void mpz_mul_si(x,y,n)
MP_INT *x;MP_INT *y; long n;
{
    MP_INT z;
    mpz_init_set_si(&z,n);
    mpz_mul(x,y,&z);
...
void
mpz_ui_pow_ui(r,v,e)
MP_INT *r; unsigned long v; unsigned long e;
{
    MP_INT b;
    mpz_init(&b);
    mpz_set_si(&b, v);
