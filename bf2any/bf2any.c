...
OLD VERSION, only does one [-]
	...
	/* Is the loop simple ?
	 * Loop dec must be one.
	 * < and > must be balanced.
	 * I only support one [-] in the loop to keep it cheap.
	 */
	inc = mov = smov = sinc = 0;
	for(i=0; i<qcnt; i++) {
	    if (qcmd[i] == '>') mov += qrep[i];
	    if (qcmd[i] == '<') mov -= qrep[i];
	    if (qcmd[i] == '+' && mov == 0) inc += qrep[i];
	    if (qcmd[i] == '-' && mov == 0) inc -= qrep[i];
	    if (qcmd[i] == '=' && mov == 0) {
		outrun(0, 0);
		return;
	    ...
	    if (qcmd[i] == '=') smov = mov;
	    if (smov) {
		if (qcmd[i] == '+' && mov == smov) sinc += qrep[i];
		if (qcmd[i] == '-' && mov == smov) sinc -= qrep[i];
	    }
	}
	/* Not simple. */
	if (mov || inc != -1) { outrun(0, 0); return; }

	/* Change all the token to their multiple conterparts
	 * Remember any '+' or '-' tokens AFTER the [-] are only
	 * run once so we add them into the set.
	 * Setting the loop variable is handled by the 'B' start code,
	 * the loop dec and loop end are deleted.
	 *
	 * Also beware that if the loop variable is zero nothing in the loop
	 * should be touched. However, keeping the jump isn't free.
	 * The only time this might be a problem is if the cell we're changing
	 * is before the start of the array.
	 * So use the BOFF constant to make this rare and have anything more
	 * than this many cells before the loop variable set conditionally.
	 */
	for(i=0; i<qcnt; i++) {
	    if (qcmd[i] == '>') mov += qrep[i];
	    if (qcmd[i] == '<') mov -= qrep[i];
	    if (mov == 0 && (qcmd[i] == '-' || qcmd[i] == '+'))
		qcmd[i] = 0;
	    if (qcmd[i] == '[') qcmd[i] = 'B';
	    if (qcmd[i] == ']') qcmd[i] = 0;
	    if (mov == smov) {
		if( qcmd[i] == '+' || qcmd[i] == '-') qcmd[i] = 0;;
	    } else if (mov < -BOFF) {
		if (qcmd[i] == '+' && qrep[i] == 1) qcmd[i] = 's';
		if (qcmd[i] == '+') qcmd[i] = 'm';
		if (qcmd[i] == '-') qcmd[i] = 'n';
	    } else {
		if (qcmd[i] == '+' && qrep[i] == 1) qcmd[i] = 'S';
	    ...
	    if (qcmd[i] == '=') { qcmd[i] = 'Q'; qrep[i] = sinc; }
	}
#endif
